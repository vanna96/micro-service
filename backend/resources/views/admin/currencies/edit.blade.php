...
@section('content')
<form method="POST" action="{{ route('admin.currencies.update', ['currency' => $currency->id]) }}" class="admin-form-page">
    @csrf
    @method('PUT')

    <div class="alert alert-border-left alert-light mb-4" role="alert">
        <i class="mdi mdi-currency-usd me-2"></i>Editing currency in tenant <strong>{{ admin_tenant_display_name($selectedTenant) }}</strong>.
    </div>
