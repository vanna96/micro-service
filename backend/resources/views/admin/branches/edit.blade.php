...
@section('content')
<form method="POST" action="{{ route('admin.branches.update', ['branch' => $branch->id]) }}" class="admin-form-page">
    @csrf
    @method('PUT')

    <div class="alert alert-border-left alert-light mb-4" role="alert">
        <i class="mdi mdi-database me-2"></i>Editing branch in tenant <strong>{{ admin_tenant_display_name($selectedTenant) }}</strong>.
    </div>
