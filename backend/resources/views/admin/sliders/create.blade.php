...
@section('content')
<form method="POST" action="{{ route('admin.sliders.store') }}" class="admin-form-page" enctype="multipart/form-data">
    @csrf

    <div class="alert alert-border-left alert-light mb-4" role="alert">
        <i class="mdi mdi-database me-2"></i>Creating slider for tenant <strong>{{ admin_tenant_display_name($selectedTenant) }}</strong>.
    </div>
