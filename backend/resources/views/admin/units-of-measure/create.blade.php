...
@section('content')
<form method="POST" action="{{ route('admin.units-of-measure.store') }}" class="admin-form-page">
    @csrf

    <div class="alert alert-border-left alert-light mb-4" role="alert">
        <i class="mdi mdi-database me-2"></i>Creating unit of measure for tenant <strong>{{ admin_tenant_display_name($selectedTenant) }}</strong>.
    </div>
