...
@section('content')
<form method="POST" action="{{ route('admin.addresses.update', ['address' => $address->id]) }}" class="admin-form-page">
    @csrf
    @method('PUT')

    <div class="alert alert-border-left alert-light mb-4" role="alert">
        <i class="mdi mdi-database me-2"></i>Editing address in tenant <strong>{{ admin_tenant_display_name($selectedTenant) }}</strong>.
    </div>
