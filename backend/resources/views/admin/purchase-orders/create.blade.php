...
@section('content')
<form method="POST" action="{{ route('admin.purchase-orders.store') }}" class="admin-form-page">
    @csrf

    <div class="alert alert-border-left alert-light mb-4" role="alert">
        <i class="mdi mdi-database me-2"></i>Creating purchase order in tenant <strong>{{ admin_tenant_display_name($selectedTenant) }}</strong>.
    </div>
