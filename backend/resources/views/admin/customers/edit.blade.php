...
@section('content')
<form method="POST" action="{{ route('admin.customers.update', ['customer' => $customer->id]) }}" class="admin-form-page" enctype="multipart/form-data">
    @csrf
    @method('PUT')

    <div class="alert alert-border-left alert-light mb-4" role="alert">
        <i class="mdi mdi-database me-2"></i>Editing customer in tenant <strong>{{ admin_tenant_display_name($selectedTenant) }}</strong>.
    </div>
